...
export async function getUserFork(
  ...
  if (testMode?.enabled) {
    ...
    console.log(
      `[TEST MODE] getUserFork: ${existingFork ? "Found" : "Not found"} fork for ${username}`
    );
...
export async function forkRepository(
  ...
  console.log(
    "[forkRepository] Response status:",
    response.status,
    response.statusText
  );
...
export async function getBranchSha(
  ...
  if (testMode?.enabled) {
    ...
    console.log(
      `[TEST MODE] getBranchSha: Returning SHA ${sha} for ${branchKey}`
    );
    return sha;
  }
...
export async function createBranch(
  ...
  if (testMode?.enabled) {
    ...
    testModeState.branches.set(branchKey, { sha });
    console.log(
      `[TEST MODE] createBranch: Created branch ${branchKey} with SHA ${sha}`
    );
    return;
  }
...
export async function branchExists(
  ...
  if (testMode?.enabled) {
    ...
    console.log(
      `[TEST MODE] branchExists: Branch ${branchKey} ${exists ? "exists" : "does not exist"}`
    );
    return exists;
  }
...
export async function getFileContent(
  ...
  if (testMode?.enabled) {
    ...
    console.log(
      `[TEST MODE] getFileContent: ${existingFile ? "Found" : "Not found"} file ${fileKey}`
    );
...
export async function createOrUpdateFile(
  ...
  if (testMode?.enabled) {
    ...
    console.log(
      `[TEST MODE] createOrUpdateFile: ${existingSha ? "Updated" : "Created"} file ${fileKey}`
    );
...
export async function createPullRequest(
  ...
  if (testMode?.enabled) {
    ...
    console.log(
      `[TEST MODE] PR from ${username}:${branchName} to ${GITHUB_CONFIG.owner}:${GITHUB_CONFIG.defaultBranch}`
    );
    return mockPR;
  }
