  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900">
      <main className="mx-auto max-w-3xl px-6 py-16 md:py-24">
        <Link 
          to="/" 
          className="mb-8 inline-flex items-center gap-2 text-sm font-medium text-zinc-600 transition-colors hover:text-orange-600"
        >
        ...
        <div className="mb-8">
          <input type="text" placeholder="search products or topics" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full rounded-lg border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-900 placeholder-zinc-400 outline-none transition-colors focus:border-orange-400 focus:ring-1 focus:ring-orange-400" />
        </div>
                    ...
                    <div className="inline-flex flex-wrap items-center justify-end gap-x-3 gap-y-1">
                      {contact.handles.length <= 2 ? (
                        contact.handles.map((handle) => (
                          <a key={handle} href={`https://x.com/${handle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm text-zinc-600 transition-colors hover:text-orange-600 md:text-base">
                            <Avatar className="h-5 w-5 flex-shrink-0">
                              <AvatarImage src={`https://unavatar.io/twitter/${handle.replace('@', '')}`} alt={handle} />
                              <AvatarFallback className="bg-zinc-100 text-[10px] text-zinc-600">{handle.slice(1, 3).toUpperCase()}</AvatarFallback>
                            </Avatar>
                            ...
                            <PopoverContent className="w-auto border-zinc-200 bg-white p-3 shadow-lg">
                              <div className="flex flex-col gap-2">
                                {contact.handles.slice(2).map((handle) => (
                                  <a key={handle} href={`https://x.com/${handle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm text-zinc-600 transition-colors hover:text-orange-600">
                                    <Avatar className="h-5 w-5 flex-shrink-0">
                                      <AvatarImage src={`https://unavatar.io/twitter/${handle.replace('@', '')}`} alt={handle} />
                                      <AvatarFallback className="bg-zinc-100 text-[10px] text-zinc-600">{handle.slice(1, 3).toUpperCase()}</AvatarFallback>
                                    </Avatar>
