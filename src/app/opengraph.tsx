...
// Regex patterns for font URL extraction
const FONT_URL_REGEX = /url\(([^)]+\.woff2[^)]*)\)/;
const GSTATIC_FONT_URL_REGEX =
  /(https:\/\/fonts\.gstatic\.com\/[^\s'")]+\.woff2)/;

export const Route = createFileRoute("/opengraph")({
  server: {
    handlers: {
      // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Complex OG image generation
      GET: async () => {
        try {
          ...
          for (const face of fontFaces) {
            const urlMatch = face.match(FONT_URL_REGEX);
            if (urlMatch?.[1]) {
          ...
          if (!fontUrl) {
            const simpleMatch = cssText.match(GSTATIC_FONT_URL_REGEX);
            if (simpleMatch?.[1]) {
